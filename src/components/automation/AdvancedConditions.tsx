...
import { 
  Split, Filter, ArrowRight, CheckCircle2, 
  User, Calendar, Clock, Tag, MessageSquare,
  ShoppingCart, DollarSign, MapPin, Globe,
  Users, BarChart, Database, Layers, Workflow,
...
export function AdvancedConditions() {
  ...
  const CurrentIcon = conditionTypes[activeCondition].icon;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              <div className={`${conditionTypes[activeCondition].color} p-2 rounded-lg`}>
                <CurrentIcon className={`h-5 w-5 ${conditionTypes[activeCondition].textColor}`} />
              </div>
              <h3 className="text-xl font-bold">{conditionTypes[activeCondition].title}</h3>
            </div>

            <div className="space-y-6">
              {conditionTypes[activeCondition].conditions.map((condition, index) => (
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <select className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 text-sm">
                      {condition.operators.map((op, i) => (
                        <option key={i}>{op}</option>
                      ))}
                    </select>
                    
                    <select className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 text-sm">
                      {condition.values.map((val, i) => (
