...
export function UseCases() {
  ...
  const useCases = {
    ...
    support: {
      ...
      image:
        "https://images.unsplash.com/photo-1549923746-c502d488b3ea?ixlib=rb-1.2.1&auto=format&fit=crop&w=1000&q=80",
    },
    education: {
    ...
    <div className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          >
            <h2 className="text-3xl font-bold mb-4">حلول مخصصة لمختلف القطاعات</h2>
        ...
        {/* Active Use Case */}
        <AnimatedUseCase useCase={useCases[activeCase as keyof typeof useCases]} />
      </div>
    </div>
  );
}
...
function AnimatedUseCase({ useCase }: { useCase: any }) {
  return (
    <motion.div
      key={useCase.title}
      initial={{ opacity: 0, y: 20 }}
    ...
    >
      ...
      <div className="relative">
        ...
        <div className="relative z-10">
          <img src={useCase.image} alt={useCase.title} className="rounded-2xl shadow-xl object-cover w-full h-96" />

          {/* Stats */}
          <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 flex gap-4">
            {useCase.stats.map((stat: any, index: number) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
      ...
      <div>
        ...
        <div className="space-y-4 mb-8">
          {useCase.features.map((feature: any, index: number) => (
            <motion.div
              key={index}
