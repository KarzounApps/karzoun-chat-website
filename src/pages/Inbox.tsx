...
import { 
  MessageSquare, Search, Filter, Bell, Settings, User, 
  MoreHorizontal, Send, Paperclip, Smile, Star, Archive, 
  Trash2, Tag, Clock, CheckCircle2, AlertCircle, Phone,
  Video, Calendar, FileText, Image, Mic, ChevronDown,
  Users, RefreshCw, Menu, X, ArrowLeft, ArrowRight,
  Eye, EyeOff, UserPlus, Flag, Bookmark, Share2, 
  HelpCircle, LogOut, Moon, Sun, Globe, Zap, Bot,
  Inbox as InboxIcon, Mail, MessageCircle, Heart, ThumbsUp, ThumbsDown
} from 'lucide-react';
        ...
        attachments: [
          {
            ...
            name: 'صورة المشكلة',
            thumbnail: 'https://images.unsplash.com/photo-1588872657578-7efd1f1555ed?ixlib=rb-1.2.1&auto=format&fit=crop&w=200&q=80'
          }
        ]
      },
      {
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                {showNotifications && (
                  <div className={`absolute left-0 mt-2 w-80 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-md shadow-lg border overflow-hidden z-20`}>
                    ...
                    <div className="max-h-96 overflow-y-auto">
                      {[1, 2, 3].map((_, i) => (
                        <div 
                          key={i} 
                          className={`px-4 py-3 ${i !== 2 ? (darkMode ? 'border-b border-gray-700' : 'border-b border-gray-200') : ''} ${i === 0 ? (darkMode ? 'bg-gray-700/50' : 'bg-blue-50') : ''}`}
                        >
                          <div className="flex items-start gap-3">
                            <div className={`p-2 rounded-full ${i === 0 ? 'bg-blue-100 text-blue-600' : i === 1 ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600'}`}>
                              {i === 0 ? <MessageSquare className="h-4 w-4" /> : i === 1 ? <UserPlus className="h-4 w-4" /> : <Bell className="h-4 w-4" />}
                            </div>
                            <div>
                              <p className={`text-sm ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>
                                {i === 0 ? 'رسالة جديدة من أحمد محمد' : i === 1 ? 'تم تعيين محادثة جديدة لك' : 'تذكير: متابعة طلب العميل'}
                              </p>
                              <p className="text-xs text-gray-500 mt-1">منذ {i === 0 ? '5 دقائق' : i === 1 ? '30 دقيقة' : 'ساعة'}</p>
                            </div>
                          </div>
                        </div>
                      ))}
              ...
              <div className="relative">
                ...
                {showSettings && (
                  <div className={`absolute left-0 mt-2 w-56 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-md shadow-lg border overflow-hidden z-20`}>
                    <div className="py-1">
                      <button 
                        onClick={() => setDarkMode(!darkMode)}
                        className={`flex items-center gap-2 w-full text-right px-4 py-2 text-sm ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}
                      >
                        {darkMode ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
                        <span>{darkMode ? 'الوضع الفاتح' : 'الوضع الداكن'}</span>
                      </button>
                      <button className={`flex items-center gap-2 w-full text-right px-4 py-2 text-sm ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}>
                      ...
                      <button className={`flex items-center gap-2 w-full text-right px-4 py-2 text-sm ${darkMode ? 'hover:bg-gray-700 text-red-400' : 'hover:bg-gray-100 text-red-600'}`}>
              ...
              <div className="relative">
                <button 
                  className={`flex items-center gap-2 ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'} rounded-full`}
                ...
                {showTeamMembers && (
                  <div className={`absolute left-0 mt-2 w-64 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-md shadow-lg border overflow-hidden z-20`}>
        ...
        <AnimatePresence>
          {showSidebar && (
            <motion.aside
              ...
              className={`fixed top-16 bottom-0 right-0 z-10 w-80 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-l overflow-hidden`}
            >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className={`text-sm truncate ${
                                ...
                                {lastMessage.senderId === 'agent' ? 'أنت: ' : ''}{lastMessage.text}
                              </p>
                  ...
                  <div className="flex items-center gap-3 cursor-pointer" onClick={() => setShowUserInfo(!showUserInfo)}>
                    <div className="relative">
                      ...
                      <div className={`absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 ${darkMode ? 'border-gray-800' : 'border-white'} ${
                        ...
                        selectedContact.status === 'busy' ? 'bg-red-500' :
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-500">
                          {selectedContact.status === 'online' ? 'متصل الآن' : 
                           selectedContact.status === 'away' ? 'غير متواجد' : 
                           selectedContact.status === 'busy' ? 'مشغول' : 
                           selectedContact.lastSeen ? selectedContact.lastSeen : 'غير متصل'}
                        </span>
                        <span className="text-xs text-gray-500">•</span>
                        <span className="text-xs text-gray-500 flex items-center gap-1">
                          {getChannelIcon(selectedConversation.channel)}
                          {selectedConversation.channel === 'whatsapp' ? 'واتساب' : 
                           selectedConversation.channel === 'instagram' ? 'انستغرام' : 
                           selectedConversation.channel === 'facebook' ? 'فيسبوك' : 
                           selectedConversation.channel === 'web' ? 'الموقع' : 'البريد'}
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        <AnimatePresence>
          {showUserInfo && selectedContact && (
            <motion.aside
              ...
              exit={{ width: 0, opacity: 0 }}
              transition={{ duration: 0.3 }}
              className={`fixed top-16 bottom-0 left-0 z-10 w-80 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-r overflow-hidden`}
            >
              <div className="h-full flex flex-col">
                    ...
                    <div className={`rounded-lg overflow-hidden border ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                      ...
                      <div className="divide-y divide-gray-200">
                        <div className={`px-4 py-3 flex justify-between ${darkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                          ...
                          <span className="text-sm">{selectedContact.name.replace(/\s+/g, '.').toLowerCase()}@example.com</span>
                        </div>
                        <div className={`px-4 py-3 flex justify-between ${darkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                    ...
                    <div className={`rounded-lg overflow-hidden border ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                      ...
                      <div className="divide-y divide-gray-200">
                        {[
                          ...
                          <div key={i} className={`px-4 py-3 ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'} cursor-pointer`}>
