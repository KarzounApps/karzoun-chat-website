...
const DEFAULT_CONFIG = {
  ...
  agents: [
    { name: 'فريق المبيعات', phone: '966500000000', avatar: AVATARS[0].url, department: 'المبيعات' }
  ],
  customCSS: '',
  customJS: '',
...
`;
  ...
  const renderPreview = () => {
              ...
              >
                {config.showAvatar && (
                  <div className="w-10 h-10 rounded-full overflow-hidden bg-white">
                    <img 
                      src={config.avatar || AVATARS[0].url} 
                      alt="Agent" 
    ...
    return (
      <div className={`relative ${previewMode === 'mobile' ? 'w-[375px] h-[667px]' : 'w-full h-[500px]'} border border-gray-200 rounded-lg overflow-hidden bg-gray-100`}>
        ...
        {/* Widget preview */}
        {config.layout !== 'bar' ? (
          <div className={`absolute ${positionClasses[selectedPosition.id]} ${getAnimationClass()}`}>
            {renderLayoutContent()}
          </div>
        ) : (
          renderLayoutContent()
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="border-t border-gray-200 pt-6">
                      ...
                      <div>
                        ...
                        />
                        <p className="mt-1 text-xs text-gray-500">
                          الرسالة التي ستظهر للزائر عند فتح نافذة المحادثة
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-gray-200 pt-6">
                      ...
                      <p className="text-xs text-gray-500">
                        اختر صورة شخصية أو قم بتحميل صورة مخصصة (قريباً)
                      </p>
                    </div>
                  </div>
                )}
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              className={`h-10 w-10 rounded border-0 ${!customThemeActive ? 'opacity-50 cursor-not-allowed' : ''}`}
                            />
                            <input
                              type="text"
                              value={config.primaryColor}
                              onChange={(e) => handleConfigChange('primaryColor', e.target.value)}
                              disabled={!customThemeActive}
                              className={`block w-full rounded-md border-gray-300 focus:border-green-500 focus:ring-green-500 ${!customThemeActive ? 'opacity-50 cursor-not-allowed' : ''}`}
                            />
                          </div>
                        </div>
                        <div>
                      ...
                      {!customThemeActive && (
                        <div className="mt-2 bg-yellow-50 border-r-4 border-yellow-500 p-3">
                          <p className="text-sm text-yellow-700">
                            لتخصيص الألوان، اختر السمة "مخصص" من القائمة أعلاه.
                          </p>
                        </div>
                      )}
                    </div>
